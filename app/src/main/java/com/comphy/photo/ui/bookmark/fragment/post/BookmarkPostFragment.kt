package com.comphy.photo.ui.bookmark.fragment.post
...
import com.comphy.photo.databinding.FragmentBookmarkPostBinding
...
class BookmarkPostFragment : Fragment() {
    ...
    private var bookmarkPostAdapter: FeedAdapter? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentBookmarkPostBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun setupObserver() {
        ...
        viewModel.isDeletePostLoading.observe(viewLifecycleOwner) {
            setBottomSheetDeletePostLoading(
                it
            )
        }
        viewModel.postResponse.observe(viewLifecycleOwner) {
            lifecycleScope.launch {
                bookmarkPostAdapter!!.submitData(PagingData.empty())
                bookmarkPostAdapter!!.submitData(it)
            }
        }
        viewModel.successResponse.observe(viewLifecycleOwner) {
            bookmarkPostAdapter!!.notifyItemChanged(it)
            binding.rvPosts.clearFocus()
        }
        viewModel.followResponse.observe(viewLifecycleOwner) {
            bookmarkPostAdapter!!.refresh()
            binding.rvPosts.clearFocus()
        }
    ...
    private fun setupWidgets(userData: UserResponseData) {
        bookmarkPostAdapter = FeedAdapter(
            userData = userData,
    ...
    private fun showBottomSheetEditPost(content: FeedResponseContentItem, position: Int) {
        ...
        (activity as BookmarkActivity).bottomSheetDialog.setContentView(bottomSheetEditPostBinding.root)
        (activity as BookmarkActivity).bottomSheetDialog.show()
    }
    ...
    private fun showBottomSheetDeletePost(postId: String, position: Int) {
        with(bottomSheetDeletePostBinding) {
            ...
            btnCancel.setOnClickListener { (activity as BookmarkActivity).bottomSheetDialog.dismiss() }
        }
        (activity as BookmarkActivity).bottomSheetDialog.setContentView(bottomSheetDeletePostBinding.root)
        (activity as BookmarkActivity).bottomSheetDialog.show()
    }
    ...
    private fun setBottomSheetDeletePostLoading(state: Boolean) {
        with(bottomSheetDeletePostBinding) {
            ...
            (activity as BookmarkActivity).bottomSheetDialog.setCancelable(!state)
