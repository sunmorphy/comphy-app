...
class FeedRecommendationFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch { viewModel.getFilteredPost() }
        setupObserver()
    }
        ...
        feedAdapter = FeedAdapter(
            ...
            videoHolder = { holder ->
                ...
                (parentFragment as FeedFragment).binding.mainView.setOnScrollChangeListener(
                    NestedScrollView.OnScrollChangeListener { _, _, _, _, _ -> holder.pausePlayer() }
                )
            }
        )
    ...
    private fun showBottomSheetEditPost(content: FeedResponseContentItem, position: Int) {
        with(bottomSheetEditPostBinding) {
            ...
            edtPostTitle.doAfterTextChanged {
                txtPostTitleMax.text = kotlin.String.format(
                    getString(com.comphy.photo.R.string.placeholder_post_title_count),
        ...
        with(bottomSheetDeletePostBinding) {
            if (state) {
                ...
                imgLoadingBtn.apply {
                    ...
                    startAnimation(activity?.loadAnim(com.comphy.photo.R.anim.btn_loading_anim))
                }
            } else {
